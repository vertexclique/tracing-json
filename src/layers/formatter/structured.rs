...
use tracing::{Level, Event, Id, Subscriber, span::Attributes};
    ...
    fn from(data: &Value) -> Result<Datatype> {
        match *data {
            Value::Object(ref map) => match map.get("type") {
                Some(d) if d == "constant" => {
                    match map.get("value") {
                        Some(v) => Ok(Datatype::Constant(v.as_str().unwrap_or_default().into())),
                        _ => return Err(StructuredError::ParseError(
                ...
                Some(d) if d == "dynamic" => {
                    match map.get("return_type") {
                        Some(v) => Ok(Datatype::Dynamic(v.as_str().unwrap_or_default().into())),
                        _ => return Err(StructuredError::ParseError(
                            "Datatype missing 'return_type' at 'dynamic'".to_string(),
                        ))
                    }
                },
    ...
    W: MakeWriter + 'static
{
    ...
    pub(crate) fun_registry: HashMap<String, FunStub>,
}
    ...
    W: MakeWriter + 'static
{
    pub fn new<'d>(format: &'d str, writer: W) -> Result<Self>
    {
        let conf: Value = serde_json::from_str(format)
            .map_err(|_| StructuredError::ParseError(
                "Config is not in json format".to_string(),
            ))?;
        ...
        match conf {
            Value::Object(ref structure) =>{
                ...
                Ok(Self { fields, make_writer: writer, fun_registry: HashMap::default() })
            },
            _ => Err(StructuredError::ParseError(
    ...
    fn structured_fields(
        ...
        self.fields
            ...
            .try_for_each(|f| {
                match &f.dtype {
                    Datatype::Constant(s) =>
                        Ok(ms.serialize_entry(&f.name, &s)?),
                    Datatype::Level =>
                        Ok(ms.serialize_entry(&f.name, &level.to_string())?),
                    Datatype::Message =>
                        Ok(ms.serialize_entry(&f.name, message)?),
                    Datatype::Dynamic(ret_type) => {
                        match ret_type {
                            r if r == "str" => {
                                let def = String::from("");
                                let res = self.fun_registry.get(&f.name).unwrap()();
                                let res = res.downcast_ref::<String>().unwrap_or(&def);
                                Ok(ms.serialize_entry(&f.name, res)?)
                            },
                            r if r == "u64" => {
                                let res = self.fun_registry.get(&f.name).unwrap()();
                                let res = res.downcast_ref::<u64>().unwrap_or(&0_u64);
                                Ok(ms.serialize_entry(&f.name, res)?)
                            },
                            r if r == "i64" => {
                                let res = self.fun_registry.get(&f.name).unwrap()();
                                let res = res.downcast_ref::<i64>().unwrap_or(&0_i64);
                                Ok(ms.serialize_entry(&f.name, res)?)
                            },
                            r if r == "bool" => {
                                let res = self.fun_registry.get(&f.name).unwrap()();
                                let res = res.downcast_ref::<bool>().unwrap_or(&false);
                                Ok(ms.serialize_entry(&f.name, res)?)
                            },
                            _ => return Err(StructuredError::ParseError(
                                "Field missing 'dtype' attribute".to_string(),
                    ...
                    Datatype::CurrentIso8601 =>
                        Ok(ms.serialize_entry(&f.name, &now.to_rfc3339())?),
        ...
        S: Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>
    {
        ...
        if let Some(span) = &current_span {
            message = format!("{} {}", self.format_span_context(span, SpanState::Event), message);
        }

        message
    }

    fn format<S>(&self, event: &Event<'_>, current_span: Option<SpanRef<S>>, mut event_visitor: JsonStorage) -> Result<Vec<u8>>
    where
...
mod tracing_json_structured_tests {
    ...
    fn parse_structured() {
        ...
        let s = Structured::new(config, std::io::stdout).unwrap();
        dbg!(s);
    }
}
